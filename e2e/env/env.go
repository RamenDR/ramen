...
// GetEnvCluster returns the cluster from the env that matches clusterName.
// If not found, it returns an empty Cluster and an error.
func GetEnvCluster(ctx types.Context, clusterName string) (types.Cluster, error) {
	env := ctx.Env()
	...
	case env.Hub.Name:
		return env.Hub, nil
	default:
		return types.Cluster{}, fmt.Errorf("cluster %s not found in environment", clusterName)
	}
}
