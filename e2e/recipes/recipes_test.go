			...
			Workflows: []*recipe.Workflow{
				{
					...
					Sequence: []map[string]string{
						replicasCheckHook, availableCheckHook, lsExecHook,
						echoExecHook, group,
					},
				},
				{
					Name: "restore",
					Sequence: []map[string]string{
						group, lsExecHook, echoExecHook, replicasCheckHook,
						availableCheckHook,
					},
				},
			},
...
func TestGenerateWithNilChecks(t *testing.T) {
	recipeConfig := &config.Recipe{
		Type:      "generate",
		CheckHook: true,
		ExecHook:  true,
	...
	assert.Contains(t, err.Error(), "no checks defined for workload")
}
...
func (w *NoHooks) GetAppName() string {
	return "nohooks-app"
}

func (w *NoHooks) GetName() string {
	return "nohooks-workload"
}

func (w *NoHooks) GetPath() string {
	return "path/to/nohooks"
}

func (w *NoHooks) GetBranch() string {
	return "main"
}

func (w *NoHooks) GetSelectResource() string {
	return "deployment"
}
...
func (w *NoHooks) GetChecks(namespace string) []*recipe.Check {
	return nil
}

func (w *NoHooks) GetOperations(namespace string) []*recipe.Operation {
	return nil
}

func (w *NoHooks) Kustomize() string {
	return ""
}

func (w *NoHooks) Health(ctx types.TestContext, cluster *types.Cluster) error {
	return nil
}

func (w *NoHooks) Status(ctx types.TestContext) ([]types.WorkloadStatus, error) {
	return nil, nil
}
