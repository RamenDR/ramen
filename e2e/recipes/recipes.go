...
func Generate(ctx types.TestContext, recipeConfig *config.Recipe) (*recipe.Recipe, error) {
	...
	return &recipe.Recipe{
		...
		Spec: recipe.RecipeSpec{
			...
			Workflows: generateWorkflows(recipeConfig, hooks),
		},
	}, nil
}
...
func prepareCheckHook(ctx types.TestContext) (*recipe.Hook, error) {
	...
	if checks = ctx.Workload().GetChecks(ctx.AppNamespace()); len(checks) == 0 {
		return nil, fmt.Errorf("failed to prepare check hooks, no checks defined for workload %q",
			ctx.Workload().GetName())
	}
...
func prepareExecHook(ctx types.TestContext) (*recipe.Hook, error) {
	...
	if operations = ctx.Workload().GetOperations(ctx.AppNamespace()); len(operations) == 0 {
		return nil, fmt.Errorf("failed to create exec hook, no operations defined for workload %q", ctx.Workload().GetName())
	}
...
func generateWorkflows(recipeSpec *config.Recipe, hooks []*recipe.Hook) []*recipe.Workflow {
	...
	backup.Sequence = buildSeq(group, checkHooks, execHooks, recipeSpec.CheckHook, recipeSpec.ExecHook, false)

	restore.Sequence = buildSeq(group, checkHooks, execHooks, recipeSpec.CheckHook, recipeSpec.ExecHook, true)
...
func buildSeq(
	...
	checkHooks, execHooks []map[string]string,
	includeCheck, includeExec bool,
	reverse bool,
) []map[string]string {
	seq := make([]map[string]string, 0)

	// restore starts with group, backup ends with group
	if reverse {
		seq = append(seq, group)
	}

	if includeCheck {
		seq = append(seq, checkHooks...)
	}

	if includeExec {
		seq = append(seq, execHooks...)
	}
