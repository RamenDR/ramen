...
func GetCurrentAndTargetCluster(client client.Client, namespace, placementName string) (string, string, error) {
	drpolicy, err := GetDRPolicy(client, DefaultDRPolicyName)
	if err != nil {
		return "", "", err
	}
	...
	return currentCluster, targetCluster, nil
}
