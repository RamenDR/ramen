...
func (d DiscoveredApp) Deploy(ctx types.TestContext) error {
	...
	if err := util.ApplyKustomization(tempDir, appNamespace, cluster.Kubeconfig); err != nil {
		return err
	}
...
func deleteDiscoveredApps(ctx types.TestContext, cluster *types.Cluster, namespace string, wait bool) error {
	...
	return util.DeleteKustomization(tempDir, namespace, cluster.Kubeconfig, wait)
}
