...
func (l *StatefulSetPodLister) getAllPodsFromStatefulSets(statefulSets []appsv1.StatefulSet, log logr.Logger) ([]ExecPodSpec, error) {
	execPods := make([]ExecPodSpec, 0)
	...
	for _, statefulSet := range statefulSets {
		if statefulSet.Status.ReadyReplicas > 0 {
			// Get all pods owned by this StatefulSet
			podList := &corev1.PodList{}
			err := l.Reader.List(context.Background(), podList, client.InNamespace(statefulSet.Namespace))
			if err != nil {
			...
			for i := range podList.Items {
				pod := &podList.Items[i]
				if IsPodOwnedByStatefulSet(pod, statefulSet.Name) {
					execPods = append(execPods, ExecPodSpec{
						PodName:   pod.Name,
						Namespace: pod.Namespace,
						Command:   cmd,
