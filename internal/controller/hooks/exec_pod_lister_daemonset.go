...
func (l *DaemonSetPodLister) getPodsFromDaemonSets(daemonSets []appsv1.DaemonSet, log logr.Logger) ([]ExecPodSpec, error) {
...
func (l *DaemonSetPodLister) getAllPodsFromDaemonSets(daemonSets []appsv1.DaemonSet, log logr.Logger) ([]ExecPodSpec, error) {
	execPods := make([]ExecPodSpec, 0)
	...
	for _, daemonSet := range daemonSets {
		if daemonSet.Status.NumberReady > 0 {
			// Get all pods owned by this DaemonSet
			podList := &corev1.PodList{}
			err := l.Reader.List(context.Background(), podList, client.InNamespace(daemonSet.Namespace))
			if err != nil {
