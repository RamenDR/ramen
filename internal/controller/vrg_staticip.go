...
func (v *VRGInstance) staticIPResourceModifierReconcile() bool {
	spec := v.instance.Spec.StaticIPTranslationSpec
	if spec == nil || len(spec.IPTranslations) == 0 {
		return false
	}
	...
	for i := range translations {
		...
		for j := range t.Networks {
			...
			for k := range net.Addresses {
				...
				if ns := t.ResourceRef.Namespace; ns != "" {
					conditions.Namespaces = []string{ns}
				}
...
func buildAnnotationPatchValue(networkName, targetIP string) (string, error) {
	...
	outerJSON, err := json.Marshal(string(innerJSON))
	if err != nil {
		return "", err
	}
...
func hasStaticIPTranslation(vrg *ramen.VolumeReplicationGroup) bool {
	return vrg.Spec.StaticIPTranslationSpec != nil &&
		len(vrg.Spec.StaticIPTranslationSpec.IPTranslations) > 0
}
...
func (v *VRGInstance) ensureResourceModifierCM(result *ctrl.Result) error {
	...
	v.staticIPResourceModifierReconcile()
