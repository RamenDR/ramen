...
func (v *VRGInstance) updateVR(pvc *corev1.PersistentVolumeClaim, volRep client.Object,
	...
	isCGEnabled := func() bool {
		_, ok := pvc.GetLabels()[ConsistencyGroupLabel]

		return ok && rmnutil.IsCGEnabled(v.instance.GetAnnotations())
	}
...
func (v *VRGInstance) validateVRValidatedStatus(
	...
	conditionMet, condState, errorMsg := isVRConditionMet(status, volrep.ConditionValidated, metav1.ConditionTrue)
	if !conditionMet {
		if errorMsg == "" {
...
func (v *VRGInstance) validateVRCompletedStatus(pvc *corev1.PersistentVolumeClaim, volRep client.Object,
	...
	conditionMet, _, errorMsg := isVRConditionMet(status, volrep.ConditionCompleted, metav1.ConditionTrue)
	if !conditionMet {
		if errorMsg == "" {
			var (
				stateString string
...
func (v *VRGInstance) validateAdditionalVRStatusForSecondary(pvc *corev1.PersistentVolumeClaim, volRep client.Object,
	...
	conditionMet, _, _ := isVRConditionMet(status, volrep.ConditionResyncing, metav1.ConditionTrue)
	if !conditionMet {
		return v.checkResyncCompletionAsSecondary(pvc, volRep, status)
	}

	conditionMet, _, errorMsg := isVRConditionMet(status, volrep.ConditionDegraded, metav1.ConditionTrue)
	if !conditionMet {
		if errorMsg == "" {
...
func (v *VRGInstance) checkResyncCompletionAsSecondary(pvc *corev1.PersistentVolumeClaim, volRep client.Object,
	...
	conditionMet, _, errorMsg := isVRConditionMet(status, volrep.ConditionResyncing, metav1.ConditionFalse)
	if !conditionMet {
		if errorMsg == "" {
	...
	conditionMet, _, errorMsg = isVRConditionMet(status, volrep.ConditionDegraded, metav1.ConditionFalse)
	if !conditionMet {
		if errorMsg == "" {
...
const (
	...
	// Found, but its value is "Unknown".
	conditionUnknown = conditionState("unknown")
	// Found and the value is "True" or "False"
...
//   - state: one of (conditionMissing, conditionUnknown, conditionKnown)
//   - errorMsg: error message describing why the condition is not met
func isVRConditionMet(status *volrep.VolumeReplicationStatus,
