...
func (v *VSHandler) ReconcileRD(
	...
	if err = v.assignRDAsOwnerToProtectedPVC(rd, rdSpec); err != nil {
		return nil, err
	}
...
func (v *VSHandler) setRDAsOwnerOfPVC(
	...
	// DeepCopy the original PVC for patch
	orig := pvc.DeepCopy()

	// Overwrite OwnerReferences with the new one
	pvc.SetOwnerReferences([]metav1.OwnerReference{*ref})

	// Patch the PVC (safely updates even if resourceVersion changed)
	if err := v.client.Patch(v.ctx, pvc, client.MergeFrom(orig)); err != nil {
		v.log.Error(err, "Failed to patch ProtectedPVC", "PVC", pvc.Name)
