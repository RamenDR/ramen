...
func (RequestsManager) RecoverRequestCreate(
	...
	restore, err := restoreRealCreate(
		objectWriter{ctx: ctx, Client: writer, log: log},
		s3Url,
		s3BucketName,
		s3RegionName,
		s3KeyPrefix,
...
func (r BackupRequest) Deallocate(
	...
	return objectWriter{ctx: ctx, Client: writer, log: log}.backupObjectsDelete(
...
func (r RestoreRequest) Deallocate(
	...
	return objectWriter{ctx: ctx, Client: writer, log: log}.restoreObjectsDelete(
...
func backupRequestCreate(
	...
	err := w.objectCreateOrUpdate(s3Url,
		s3BucketName,
		s3RegionName,
		s3KeyPrefix,
		secretKeyRef,
...
func (w objectWriter) objectCreateOrUpdate(s3Url string,
	s3BucketName string,
	s3RegionName string,
	s3KeyPrefix string,
	secretKeyRef *corev1.SecretKeySelector,
...
func (w objectWriter) checkBSLStatus(
	...
	w.log.V(1).Info("Check BSL status")
...
func (w objectWriter) checkBSLAvailability(bsl *velero.BackupStorageLocation) error {
	w.log.V(1).Info("checkBSLAvailability")
	...
	w.log.Info("Check BSL availability")
