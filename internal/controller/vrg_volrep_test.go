...
var _ = Describe("VolumeReplicationGroupVolRepController", func() {
	...
	Context("Create VRG and validate VM status", func() {
		...
		It("Primary VRG: VM with protection label in VRG spec should trigger validation and report no-conflict", func() {
			...
			Expect(condition.Reason).To(Equal("NoConflictDetected"))
		})
		...
		It("Primary VRG: VM missing protection label but listed in VRG spec should report conflict", func() {
			...
			}, vrgtimeout, vrginterval).Should(
				Equal(metav1.ConditionFalse), "while waiting for VM conflict validation to complete")
		...
		It("Secondary VRG: VM with protection label in VRG spec should trigger validation and report conflict", func() {
			...
			Eventually(func() metav1.ConditionStatus {
				...
				return metav1.ConditionUnknown
			}, vrgtimeout, vrginterval).Should(Equal(metav1.ConditionFalse))
		...
		It("Secondary VRG: VM without protection label in VRG spec should trigger validation and report conflict", func() {
			vrgTestCase = newVRGTestCaseCreate(1, vrgEmptySCTemplate, true, false)
			vrgTestCase.namespace = ramenConfig.RamenOpsNamespace
			vrgTestCase.replicationState = "secondary"
			vrgTestCase.protectedVMs = []string{"vm-1-test"}
		...
		It("Primary VRG: Name of VM existing differs from VRG spec results in conflict", func() {
			createVM("vm-1-test")
		...
		It("Primary VRG-Secondary VRG: After VM deletion, primary shows conflict, secondary shows no-conflict", func() {
			createVM("vm-1-test")
			vmNamespacedName1 := types.NamespacedName{
				Name:      "vm-1-test",
				Namespace: "default",
			...
			secondaryVrgTestCase := newVRGTestCaseCreate(1, vrgEmptySCTemplate, true, false)
			secondaryVrgTestCase.namespace = ramenConfig.RamenOpsNamespace
			secondaryVrgTestCase.replicationState = "secondary"
			secondaryVrgTestCase.protectedVMs = protectedVMs
			secondaryVrgTestCase.VRGTestCaseStart()
