		...
		Context("ClusterDataConflictSecondary scenarios", func() {
			...
			It("validates VolRep PVC conflict detection (RBD Secondary)", func() {
				...
				Eventually(func() bool {
					...
					for _, condition := range vrg.Status.Conditions {
						if condition.Type == "NoClusterDataConflict" &&
				...
				for i := range vrg.Status.Conditions {
					if vrg.Status.Conditions[i].Type == "NoClusterDataConflict" {
						conflictCondition = &vrg.Status.Conditions[i]
