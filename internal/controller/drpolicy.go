...
func drClusterSecretsDeploy(
	...
	objectsToAppend := drClusterPolicyObjectsToDeploy(rmnCfg, log)
...
//nolint:funlen
func drClusterPolicyObjectsToDeploy(hubOperatorRamenConfig *rmn.RamenConfig, log logr.Logger) []*cpcv1.ObjectTemplate {
	...
	if err != nil {
		log.Error(err, "failed to create ConfigMap for DRClusterOperator")
	...
	if err != nil {
		log.Error(err, "failed to convert OperatorGroup to unstructured")
