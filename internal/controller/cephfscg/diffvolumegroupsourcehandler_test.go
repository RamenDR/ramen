...
func updateDiffVGSReadyByName(name string) {
	retryErr := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		ready := true
		vgs.Status = &vgsv1.VolumeGroupSnapshotStatus{
			ReadyToUse: &ready,
		}
...
func cleanupAllDiffVGS(ownerName string) {
	...
	Eventually(func() int {
		list := &vgsv1.VolumeGroupSnapshotList{}
...
func createPreviousVGS(name, ownerName string) {
	...
	vgs := &vgsv1.VolumeGroupSnapshot{
		...
		Spec: vgsv1.VolumeGroupSnapshotSpec{
			VolumeGroupSnapshotClassName: &diffVGSCName,
			Source: vgsv1.VolumeGroupSnapshotSource{
	...
	retryErr := retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		vgs.Status = &vgsv1.VolumeGroupSnapshotStatus{ReadyToUse: &ready}
