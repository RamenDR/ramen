...
func (mwu *MWUtil) CreateOrUpdateVRGManifestWork(
	...
	schedule *string, replClassSelector *metav1.LabelSelector) error {
	...
	manifestWork, err := mwu.generateVRGManifestWork(name, namespace, homeCluster,
		s3Endpoint, s3Region, s3SecretName, pvcSelector, schedule, replClassSelector)
	if err != nil {
		return err
	}
...
func (mwu *MWUtil) generateVRGManifestWork(
	...
	pvcSelector metav1.LabelSelector, schedule *string,
	...
	vrgClientManifest, err := mwu.generateVRGManifest(name, namespace, s3Endpoint,
		s3Region, s3SecretName, pvcSelector, schedule, replClassSelector)
	if err != nil {
