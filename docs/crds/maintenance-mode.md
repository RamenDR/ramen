...
Cluster-scoped resource created by Ramen on managed clusters to coordinate with
storage backends during DR operations. Storage backends reconcile this resource
to prepare for failover and report readiness back to Ramen.

**Lifecycle:** Automatically created by Ramen when maintenance mode is required.
Reconciled by storage provider controllers. Deleted when maintenance is
complete.
