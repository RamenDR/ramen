...
`drenv cleanup` and `drenv setup` cycles. This preserves the in-memory metadata
cache, which provides near 100% hit rate after the first run.

- `drenv setup`: Creates cache containers if they don't exist, or recreates them
  if the configuration has changed. Running containers with current
  configuration are not touched.
- `drenv cleanup`: Keeps cache containers running. Previously cleanup removed
  the containers, discarding the in-memory cache and causing hundreds of
  unnecessary cache misses on the next start.
...
This allows minikube VMs to access the pull-through cache containers running on
the host.
...
This allows minikube VMs to access all podman published ports, including the
registry cache.
...
- **misses**: cache lookups where the item was not cached (fetched from
  upstream)
...
in-memory metadata cache. This command is useful for development and testing of
the registry cache.
