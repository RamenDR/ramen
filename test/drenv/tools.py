...
invokes the version pinned in tools.mod using `go tool -modfile=tools.mod kustomize`.
"""
...
def tools_mod():
    """Return the path to tools.mod at the repository root."""
    return os.path.join(repo_root(), "tools.mod")
...
def install_kustomize(install_dir):
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(f"""#!/bin/sh
cd {root!r} || exit 1
exec go tool -modfile={modfile!r} kustomize "$@"
""")
    os.chmod(path, 0o755)
...
def setup(install_dir=None):
    ...
    if go_version != required:
        raise RuntimeError(
            ...
            f"required version {REQUIRED_VERSION}; update tools.mod tool directive"
        )
