...
def deploy_service(namespace, dst, src):
    ...
    kubectl.apply(f"--kustomize={_TEST_DATA}/{namespace}/dst", context=dst)
    print(f"Deploying namespace '{namespace}' on clusters '{src}'")
    kubectl.apply(f"--kustomize={_TEST_DATA}/{namespace}/src", context=src)
...
def undeploy_service(namespace, dst, src):
    ...
    kubectl.delete(
        f"--kustomize={_TEST_DATA}/{namespace}/dst",
        "--wait=false",
        "--ignore-not-found",
        context=dst,
    )
    ...
    kubectl.delete(
        f"--kustomize={_TEST_DATA}/{namespace}/src",
        "--wait=false",
        "--ignore-not-found",
        context=src,
    )
...
def wait_for_service_import(cluster, namespace):
    print(
        f"Waiting until serviceimport '{namespace}/{SERVICE}' exists in cluster '{cluster}'"
    )
    kubectl.wait(
    ...
    print(
        f"Waiting until serviceimport '{namespace}/{SERVICE}' has clusters in cluster '{cluster}'"
    )
    kubectl.wait(
        f"serviceimports/{SERVICE}",
