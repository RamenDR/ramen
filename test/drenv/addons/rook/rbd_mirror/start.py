...
def start(cluster1, cluster2, debug=False):
    """
    Set up rbd mirroring between two clusters.
    """
    if debug:
        enable_debug_logs(cluster1)
        enable_debug_logs(cluster2)
    else:
...
def fetch_secret_info(cluster):
    ...
    print(
        f"Waiting until cephblockpool 'rook-ceph/{POOL_NAME}' has mirroring info site name"
    )
    kubectl.wait(
...
def disable_debug_logs(cluster):
...
def configure_mirroring(cluster, peer_info):
    print(f"Applying rbd mirror secret in cluster '{cluster}'")

    template = _template("rbd-mirror-secret.yaml")
    yaml_str = template.substitute(peer_info)
    kubectl.apply(
        "--filename=-",
        "--namespace=rook-ceph",
        input=yaml_str,
...
def wait_until_ready(cluster):
    print(
        f"Waiting until cephrbdmirror 'rook-ceph/my-rbd-mirror' exists in cluster '{cluster}'"
    )
    kubectl.wait(
    ...
    print(
        f"Waiting until cephrbdmirror 'rook-ceph/my-rbd-mirror' is ready in cluster '{cluster}'"
    )
    kubectl.wait(
        "cephrbdmirror/my-rbd-mirror",
...
def _watch_pool_mirroring_status(cluster, timeout=180):
    """
    ...
    while True:
        ...
        with closing(watcher):
            for line in watcher:
                ...
                if _mirroring_is_healthy(summary):
...
def _mirroring_is_healthy(summary):
    return (
...
def _restart_rbd_mirror_daemon(cluster):
