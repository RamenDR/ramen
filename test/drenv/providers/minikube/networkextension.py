...
def _parse_line(line):
    """
    Parse a line and return fields if it's an extension line, None otherwise.

    Skip lines that don't have 6 tab-separated fields, and skip the known
    header line that starts with "enabled\t".
    """
    fields = line.split("\t")
    if len(fields) != 6:
        return None
